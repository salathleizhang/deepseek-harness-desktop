...
import { desktopArtifactBaseName, desktopUpdateMetadataFilename } from '../scripts/desktop-auto-update-environment.mjs'
...
describe('desktop upload plan', () => {
  it('validates macOS artifacts and puts channel metadata last', async () => {
    ...
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'DeepSeek.Harness-arm64.dmg',
      'DeepSeek.Harness-arm64.zip',
      'DeepSeek.Harness-arm64.zip.blockmap',
      'latest-mac.yml',
    ])
  ...
  it('rejects stale architecture metadata and modified updater bytes', async () => {
    ...
    const zipPath = join(paths.artifactsRoot, 'DeepSeek.Harness-arm64.zip')
    ...
    await writeFile(metadataPath, `${JSON.stringify({
      version: '1.2.3',
      files: [{
        url: 'DeepSeek.Harness-x64.zip',
        size: Buffer.byteLength(x64),
        sha512: digest(x64),
      }],
    })}\n`)
    await expect(createDesktopUploadPlan('mac-arm64', paths)).rejects.toThrow(/Harness-arm64\.zip/u)
  })
})
