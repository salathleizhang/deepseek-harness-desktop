...
The metadata must sit beside the payloads it names, under the filenames electron-builder emitted: `DeepSeek.Harness-arm64.dmg` for the manual installer, `DeepSeek.Harness-arm64.zip` plus its `.blockmap` for the macOS updater payload, and `DeepSeek.Harness-x64.exe` for Windows, whose NSIS blockmap is embedded. None of those names carries the version, so the READMEs' `releases/latest/download/<name>` links keep resolving across releases; the metadata's own `version` field and its `files[].url` entries are what bind a payload to a release. [`desktopArtifactBaseName`](../../apps/desktop/scripts/desktop-auto-update-environment.mjs) owns that stem, and electron-builder's `artifactName` expands to the same name. The metadata carries each payload's size and SHA-512, and the upload plan rejects a mismatch before it reads credentials.
